...
묶기 전에 **작은 조각을 가까운 요소에 붙입니다.** 전구의 빛살이나 별의 반짝임처럼
떨어져 있지만 그 아이콘의 일부인 장식을 살리기 위한 단계입니다. 거리는 요소 크기에
비례해 잡으므로 해상도가 달라져도 같게 동작합니다.

그리고 **박스가 깊이 겹치는 조각끼리 합칩니다.** 막대그래프 위에 떠 있는 화살표처럼
떨어져 있지만 한 요소인 경우가 이어집니다. 시트에 나란히 놓인 서로 다른 요소는 박스가
스치더라도 겹친 넓이가 작아 합쳐지지 않습니다.

묶은 뒤에는 **끝까지 붙지 못한 조각 중 유독 작은 것을 버립니다.** 기준을 절대 크기가 아니라 다른 요소들의
...
| `--min-relative-area R` | 붙지 못하고 남은 조각 중, 다른 요소들의 중앙값 대비 이 비율보다 작은 것을 버립니다 (기본 0.08). 이름표 같은 것이 여기서 걸러집니다. `0` 이면 끕니다 |
| `--attach R` | 요소 크기 대비 이 거리 안의 작은 조각을 그 요소에 붙입니다 (기본 0.05). 전구 빛살이나 별 반짝임처럼 떨어져 있지만 아이콘의 일부인 장식을 살립니다 |
| `--no-merge-overlapping` | 박스가 깊이 겹치는 조각을 합치지 않습니다 |
...
- **장식이 멀리 떨어진 아이콘**. 기본 거리(요소 크기의 5%)보다 멀면 붙지 않고 버려집니다.
  미리보기에서 빠진 것이 보이면 "장식 붙이는 거리"를 올리세요. 다만 너무 올리면 옆의
  설명 글씨까지 딸려 들어옵니다.
