...
한 아이콘이 여러 조각으로 흩어져 따로 잡히면, 분할 방식을 **격자 분할** 로 바꾸고
**자동** 을 누르세요. 배치를 보고 열과 행을 채워 줍니다.
...
# 격자로 배치된 시트 - 크기를 직접 지정하거나 자동으로 추정
python -m imgcrop sheet.png -o out --mode grid --grid 4x3
python -m imgcrop sheet.png -o out --grid auto
...
| `trim` | 나누지 않고 바깥 여백만 제거 (`--mode none`) |
...
| `grid` | 지정한 열 x 행으로 균등 분할한 뒤 칸마다 여백 제거. 크기를 `auto` 로 두면 배치를 보고 추정합니다 |
| `none` | 나누지 않고 바깥 여백만 제거. 배경이 아닌 픽셀 전부를 감싸는 한 장이 나온다 |

`none` 은 아래에 이어지는 요소 묶기 단계를 아예 거치지 않습니다. 요소로 셀 필요가
없는데 최소 크기나 작은 조각 버리기 같은 필터를 통과시키면, 점 하나나 얇은 선이
"요소가 아니다"라고 걸러지면서 그만큼 크롭이 그림 안쪽을 파고듭니다. 그래서 배경
마스크의 경계를 그대로 씁니다. 잡티 제거(`--denoise`)만 앞단에서 적용됩니다.
배경 한 가지 색으로만 이루어진 이미지는 잘라낼 여백이 없다고 보고 원본 크기를
그대로 돌려줍니다.

투명 PNG를 넣으면 알파 채널을 기준으로 자릅니다. **이미 배경이 투명한 이미지라면
누끼(`--cutout`)를 켤 이유가 없습니다.** 알파가 곧 누끼라서, 켜 봐야 알파 8 이하의
거의 안 보이는 픽셀만 지워집니다. 잘라내기 모드의 누끼 마스크에는 잡티 제거를
적용하지 않습니다 — 잡티 제거는 점 하나 때문에 크롭이 넓어지는 걸 막으려고
있는 것이지, 칠해진 픽셀에 구멍을 내라는 뜻이 아니기 때문입니다.
...
| `--denoise` | 잡티 제거 반경(px). `0`이면 끔 (기본 1) |
| `--grid CxR\|auto` | 격자 크기, 예: `4x3`. `auto` 면 요소 배치를 보고 추정합니다 |
...
- **장식이 멀리 떨어진 아이콘**. 기본 거리(요소 크기의 5%)보다 멀면 붙지 않고 버려집니다.
  ...
  설명 글씨까지 딸려 들어옵니다.
- **한 아이콘이 여러 조각으로 흩어진 시트**. 마이크와 그 옆의 음파, 낱낱이 떨어진 파형 막대처럼
  조각들이 서로 비슷한 크기로 떨어져 있으면 각각 다른 요소로 잡힙니다. 조각 사이 거리가
  옆 아이콘까지의 거리와 겹치기 때문에 거리만으로는 가릴 수 없습니다.
  이럴 때는 `--grid auto` 를 쓰세요. 배치의 규칙성을 이용하므로 정확히 칸마다 하나씩 나옵니다.
