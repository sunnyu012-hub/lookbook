...
describe('예전 기록이 그대로 반영된다', () => {
  ...
  it('스키마 버전이 10 이다', () => {
    ...
    expect(createDefaultState().version).toBe(10)
  })
})
...
describe('자동 컬렉션', () => {
  it('여덟 가지가 있고 id 가 겹치지 않는다', () => {
    expect(AUTO_COLLECTIONS).toHaveLength(8)
    expect(new Set(AUTO_COLLECTIONS.map((c) => c.id)).size).toBe(8)
