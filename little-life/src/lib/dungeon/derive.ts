...
/**
 * 태오한테 안 듣고 스스로 알아보는 데 필요한 금속 조각 수.
 *
 ...
 *
 * 세 번째만 길이 둘이다. 태오한테 듣거나, 금속 조각을 몇 개 모아
...
export function clueViews(state: AppState): ClueView[] {
  return [
    ...
    },
    {
      ...
      note: '문양이 그 돌조각과 같다.',
      // 길이 둘이다. 태오한테 듣거나, 금속 조각을 몇 개 모아
      // 스스로 알아보거나.
      //
      // 태오 이야기 하나만 두면 던전이 "매일 사람한테 말 걸기" 뒤에
      // 잠긴다. 친밀도는 대화 하루 +2 가 주력이고 이야기는 순서대로만
      // 열려서, 아무리 많이 놀아도 안 빨라진다 — 이 앱이 처음부터
      // 안 만들겠다고 한 그 구조다.
      //
      // 그렇다고 태오 이야기를 빼지 않는다. 듣고 가는 쪽이 더 좋은
      // 길이다. 유일한 길이 아니게만 한다.
      found:
