 ...
 *
 * 15/30/60 도 여전히 모자랐다. 재보니 하루 세 개를 해도 45코인이라
 * 가게 중앙값(160)이 나흘치였고, 가격 붙은 107개를 다 사는 데
 * 스물두 달이 걸렸다. 만든 사람도 도감을 못 채우는 속도였다.
 *
 * 주간 목표를 수입으로 치면 괜찮아 보이는데, 그건 한 주 안에
 * 몰아서 해야 들어오는 돈이라 평소에는 없는 셈이다.
 * 늘 들어오는 건 퀘스트뿐이고, 그 값이 곧 체감 물가다.
 *
 * 가게는 하루에 41~52칸을 진열한다 — 107개 중 절반쯤이 매일 보인다.
 * 살 게 없어서 못 사는 게 아니라 돈이 없어서 못 사는 것이라,
 * 코인이 사실상 유일한 속도 조절기다.
 */
