...
/**
 * 퀘스트를 끝내면 조금씩 쌓이는 것. 화면에서는 **등불**이라고 부른다.
 *
 * 잠든 돌문 안쪽에서만 쓴다 — 처음 가는 방에 들어갈 때와 자리를 조사할 때.
 * 날이 바뀐다고 다시 차지 않는다. 오르는 길은 퀘스트 완료 하나뿐이다.
 * 그래서 화면에서도 "오늘" 이라는 말을 붙이지 않는다 — 채석장의 하루 3번과
 * 헷갈리는 순간, 안 차는 게 고장으로 보인다.
 *
 * 퀘스트를 하는 데도, 정원이 자라는 데도 필요하지 않다 —
 * 등불이 0이어도 오늘 할 수 있는 일은 하나도 줄지 않는다.
 */
