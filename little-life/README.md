...
- 도감 240개 중 **234개**에 진짜 그림이 있다 (트로피 6개 · 재료 8개 포함해 파일 248장)
- 3개(작은 수납장 · 비밀 서랍장 · 별빛 화분)는 아직 이모지로 그린다
- 그림도 이모지도 없는 **3개**(푹신한 빈백 · 미니 식탁 · 별자리 러그)만 실루엣이고, 방에는 아직 못 놓는다
...
```bash
npm run assets:extract    # assets/source-sheets/*.png → public/assets/collection/<id>.webp
npm run assets:manifest   # 분류 폴더로 옮기고 src/data/asset-manifest.json 갱신
npm run assets:audit      # 빠진 것 · 깨진 경로 · 중복 점검 → reports/asset-audit.md
```
