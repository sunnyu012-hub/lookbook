...
- 시트에 없던 트로피 5개는 직접 그렸다 (`assets/drawn/*.svg`)
...
원본 시트를 자르는 것부터 카탈로그에 붙기까지 세 단계다. 전부 다시 돌릴 수 있다.

```bash
npm run assets:extract    # 원본 시트 → public/assets/collection/<id>.webp
npm run assets:draw       # 직접 그린 SVG → webp  (assets/drawn/*.svg)
...
- **원본 시트는 고치지 않는다.** `assets/source-sheets/` 에 투명 PNG 그대로 두고,
  자를 때마다 여기서 다시 뽑는다. 시트 15장이 40MB라 저장소에는 넣지 않았다
  (`.gitignore`). 넣고 싶으면 그 줄만 지우면 된다.
...
지금 남아 있는 예외는 트로피 다섯 개다. 아래 참고.

#### 직접 그린 5개 (임시)

트로피 다섯 개는 어느 시트에도 없어서 임시로 그려뒀다.
**그림이 오면 바꾼다.** 가구 여섯 개도 그렇게 바꿨다.

원본은 `assets/drawn/<id>.svg` 다. SVG 를 고치고 `npm run assets:draw` 를 다시 돌리면
게임 안 그림도 따라 바뀐다. 그리는 데 크롬을 쓰는데(playwright),
그림 다시 그릴 때만 필요해서 `dependencies` 에 넣지 않았다.

시트에서 온 그림보다 단순하다. 색과 선 굵기는 맞췄지만 붓질까지 흉내내지는 않았다.

**그림이 들어오면 그린 것은 뺀다.** 빈백·미니 식탁·별자리 러그·수납장·서랍장·별빛 화분
여섯 개는 처음엔 그려서 채웠다가, 나중에 시트(`extra.png`)가 와서 진짜 그림으로 바꿨다.
