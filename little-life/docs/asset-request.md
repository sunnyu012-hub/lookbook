...
| 항목 | 값 |
| --- | --- |
| 원본 | `assets/source-sheets/npc-faces-a.webp` (6명) · `npc-faces-b.webp` (13명 중 12명) · `npc-face-*.webp` (한 명씩 6장) |
...
**남지호는 나중에 한 장짜리 그림으로 갈아끼웠다.** 시트 B 에도 지호 얼굴이
있지만 그건 안 쓴다 — `SHEET_B` 의 그 자리는 `None` 으로 비워뒀다.
목록에서 아예 빼지 않는 건 그게 시트를 왼쪽 위부터 세는 순서라서,
한 칸을 빼면 뒤 사람이 전부 한 칸씩 밀리기 때문이다.
