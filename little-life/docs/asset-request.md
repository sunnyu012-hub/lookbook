...
## UPDATE G.5 — 도시 지도 그림 한 장

지도 화면이 카드 목록에서 **그림 한 장 + 그 위에 얹는 상태**로 바뀌었다.
코드는 다 붙어 있고, **그림 파일 하나만 오면 그날부터 그 지도가 뜬다.**
그때까지는 예전 카드 목록이 그대로 나온다 (그림이 없으면 자동으로 되돌아간다).

| 항목 | 값 |
| --- | --- |
| 파일 | `public/assets/city/city_map_base.png` |
| 크기 | **1024 × 1536** (세로 2:3). 더 커도 되지만 비율은 지킨다 |
| 형식 | PNG. 여기는 배경 그림이라 **투명 아니어도 된다** |
...
1. 파일을 `public/assets/city/city_map_base.png` 에 넣는다.
2. `?dev=map` 으로 열어 누르는 자리를 눈으로 본다.
3. `CITY_MAP_REGIONS` 의 좌표를 그림에 맞춘다 (구조는 그대로, 숫자만).
4. `npx vitest run src/lib/__tests__/citymap.test.ts` — 자리표가 서로
   안 먹히는지 · 라벨이 자기 상자 안에 있는지 자동으로 잡아준다.
