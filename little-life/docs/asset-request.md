...
| 크기 | **853 × 1844** (폰 화면 비율). 비율은 코드가 그림에서 읽으니 다음 판은 달라도 된다 |
| 형식 | webp 357KB. png 로 넣으면 3MB 가 되는데, 첫 실행에 미리 받는 파일이라 그 차이가 폰에서 기다리는 시간이 된다 |
...
1. ✅ webp 로 바꿔 넣었다 (크기는 그대로).
2. ✅ `MAP_CANVAS` 를 그림 크기로 맞췄다 — 화면의 지도 칸이 이 비율을 쓴다.
3. ✅ `?dev=map` 으로 누르는 자리를 켜고 좌표를 그림에 맞췄다.
4. ✅ 라벨 없는 판이라 이름표는 코드가 얹는다 (한글로, 상태·새 소식과 함께).
5. ✅ `npx vitest run src/lib/__tests__/citymap.test.ts` 통과.

**그림을 다시 그리면** 이 순서를 그대로 반복한다. 비율이 달라져도
`MAP_CANVAS` 만 고치면 화면이 따라간다. 동네 배치가 크게 달라지면
`CITY_MAP_REGIONS` 를 전부 다시 재야 한다.

---

## 사람 얼굴 스물넷 — ✅ 전부 받았다

도시 사람 자리에는 이모지 하나(☕ 🏃 🧶)가 서 있었다. 여섯 명일 때는 그걸로도
구분이 됐는데 스물넷이 되니 아침 카페 거리에 컵과 책과 가방이 나란히 놓일 뿐
누가 누군지 읽히지 않았다. 그림이 오니 이름을 읽기 전에 사람이 먼저 보인다.

| 항목 | 값 |
| --- | --- |
| 원본 | `assets/source-sheets/npc-faces-a.webp` (6명 중 5명) · `npc-faces-b.webp` (13명 중 12명) · `npc-face-*.webp` (한 명씩 7장) |
| 잘라내기 | `npm run assets:npcs` → `public/assets/npcs/<NPC_ID>.webp` |
| 크기 | 256 × 256 정사각형 · 얼굴이 가운데 · 배경 투명 · 한 장 20KB 안쪽 |
| 화면 | NPC 시트 머리 64 · 가게 · 이야기 56 · 지도 얼굴칸 40 · 동네 시트 칩 26 · 퀘스트 카드 18 |

### 받은 스물넷

윤하루 · 윤태오 · 오미래 · 서이안 · 한도윤 · 차세라 · 고은채 · 서민지 · 박준 ·
조현우 · 이하린 · 김재희 · 최라온 · 남지호 · 장우식 · 문해인 · 배수아 · 류선재 ·
강연주 · 신유나 · 정시우 · 임소라 · 유정원 · 강유현

이모지로 서 있는 사람은 이제 없다. 그래도 이모지로 떨어지는 길은
그대로 둔다 — 사람이 늘면 그림보다 먼저 도시에 서는 때가 반드시 온다.

**남지호와 신유나는 나중에 한 장짜리 그림으로 갈아끼웠다.** 시트에도 두
사람 얼굴이 있지만 그건 안 쓴다 — `SHEET_A` · `SHEET_B` 의 그 자리는
`None` 으로 비워뒀다. 목록에서 아예 빼지 않는 건 그게 시트를 왼쪽 위부터
세는 순서라서, 한 칸을 빼면 뒤 사람이 전부 한 칸씩 밀리기 때문이다.

### 이름이 안 적혀 있던 두 장 — 둘 다 확인됐다 ✅

한 명씩 그려진 넉 장 중 소라와 재희는 그림에 이름이 있었고, 나머지 두 장은
없었다. 남은 자리로 맞췄다 — 얼굴이 없던 사람이 셋(장우식 · 차세라 · 강유현)
뿐이었으니까. 나중에 사용자가 둘 다 맞다고 확인해줬다.

- 수건 걸친 남자 = **장우식** ✅
- 와인색 코트 여자 = **차세라** ✅

다음에 이름 없는 그림이 또 오면 `scripts/extract-npc-faces.py` 의 `SINGLES`
한 줄을 바꾸고 `npm run assets:npcs` 를 다시 돌리면 된다.

### 다음 얼굴이 올 때 하는 일

1. 시트를 `assets/source-sheets/` 에 넣는다 (webp. png 는 무겁다).
2. `scripts/extract-npc-faces.py` 의 이름 목록에 id 를 적는다 — 사람 이름이
   아니라 **NPC id** 다. 초반 여섯은 둘이 다르다 (`MINA` = 윤하루).
3. `npm run assets:npcs`
4. `src/lib/city/portraits.ts` 의 `DRAWN` 에 id 한 줄을 더한다.
5. `npx vitest run src/lib/__tests__/npc-portraits.test.ts` —
   "스물넷 다 얼굴이 있다" 줄이 사람 수를 세고 있으니 같이 고친다.

한 명씩 그려진 그림은 상반신이라 얼굴이 작고 가운데 있지도 않다.
`FACE_BOX` 에 자리를 눈으로 보고 적어야 한다 (시트의 작은 그림들은
머리가 그림 폭만큼 커서 위에서 정사각형으로 자르면 그대로 얼굴이 된다).
