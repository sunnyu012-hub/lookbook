...
| 항목 | 값 |
| --- | --- |
| 원본 | `assets/source-sheets/npc-faces-a.webp` (6명) · `npc-faces-b.webp` (13명) · `npc-face-*.webp` (한 명씩 4장) |
...
`강유현 · 29 · 편의점 야간 / Creature 특화 혼혈`. 조용하고 침착하며 이상한
일에도 잘 놀라지 않는다. 밤에만 밤의 거리에 서 있다. 지금은 🌘 이모지로
그대로 서 있다 — 얼굴이 없다고 도시에서 빼지는 않는다.

### 이름이 안 적혀 있던 두 장

한 명씩 그려진 넉 장 중 소라와 재희는 그림에 이름이 있었고, 나머지 두 장은
없었다. 남은 자리로 맞췄다 — 얼굴이 없던 사람이 셋(장우식 · 차세라 · 강유현)
뿐이었고, 공원 관리인 차림의 나이 든 남자와 BAR 사장 나이대의 여자가 각각
그 둘에 걸린다.

- **수건 걸친 남자 = 장우식** — 사용자가 확인해줬다. ✅
- 와인색 코트 여자 = 차세라 — 아직 추정이다.

**아니면 `scripts/extract-npc-faces.py` 의 `SINGLES` 한 줄만 바꾸고
`npm run assets:npcs` 를 다시 돌리면 된다.**
...
5. `npx vitest run src/lib/__tests__/npc-portraits.test.ts` —
   "강유현만 아직 이모지" 줄이 깨지면 그 줄도 같이 고친다.
