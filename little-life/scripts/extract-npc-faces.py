...
# 남은 자리로 맞췄다 — 스물넷 중 얼굴이 없던 사람이 셋(장우식·차세라·강유현)
# 뿐이었고, 공원 관리인 차림의 나이 든 남자와 BAR 사장 나이대의 여자가
# 각각 그 둘에 걸린다. 수건을 걸친 쪽이 장우식인 건 사용자가 확인해줬다.
# 세라 쪽은 아직 추정이라, 아니면 이 한 줄만 바꾸면 된다.
SINGLES = {
