...
import { currentUserId } from './repositories/base'
...
export { NotSignedInError } from './repositories/base'
...
export const checkinRepository = {
  ...
  async save(input: CheckinInput, ctx?: ScoreContext): Promise<Checkin> {
    if (!supabase) return localStore.upsert(input, ctx)

    const userId = await currentUserId()
    const { data, error } = await supabase
      .from(CHECKINS_TABLE)
      .upsert(inputToRow(input, userId, ctx), { onConflict: 'user_id,date' })
      .select()
      .single()
